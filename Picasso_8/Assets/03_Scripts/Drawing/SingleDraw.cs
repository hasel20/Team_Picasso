...
public class SingleDraw : MonoBehaviour
{
    //기본 신체 세팅 
    ...
    void Start()
    {
        ///오른손에 들어있는 라인 으로 붓막대 만들꺼임. 
        ///b_stick = rhand_R.GetComponent<LineRenderer>();
        ///b_stick.positionCount = 0; // 시작에는 0으로 비워줌 ! 
        //---------------------
    ...
    void Hold_Brush()
    {
        ...
        if (brush)
        {
            ...
            if (hits.Length > 0)
            {
                ...
                //Select_Figure(hits[0].gameObject); //잠시 보류 
            }
        }
    }
    ...
    void GrapLine()
    {
        if (obj == null) //잡은게 없음면 --잡기
        {
            if (OVRInput.GetDown(OVRInput.Button.PrimaryHandTrigger, OVRInput.Controller.LTouch))
            {
                ...
                if (coll.Length > 0 && coll[0].gameObject.layer != LayerMask.NameToLayer("Player"))
                {
                    ...
                    //잡은 아이가 라인이라면 라인 움직일 준비해주자
                    if (coll[0].gameObject.CompareTag("Line"))
                    {
                        Empower();// 점마다 빈오브젝트를 부여하는 함수 
                    }
                    else
                    {
        ...
        else if (obj != null)//잡은게 있으면 --놓기
        {
            ...
            if (OVRInput.GetDown(OVRInput.Button.PrimaryHandTrigger, OVRInput.Controller.LTouch))
            {
                SetOff();//만약 라인이였을떈 u_obj를 e_obj로 바꿔주고 다시 베이크 해주기 
